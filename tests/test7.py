    ...
    for i in range(grid_width):
        for j in range(grid_height):
            if is_visited(i,j) == False:
                grid[i][j]['color'] = (128, 128, 128)  # Grey color in RGB
                visited_cells.append([i,j])
                
                dir_lst = ["top", "right", "bottom", "left"]
                while(remove_wall(i,j,dir_lst[rand.randint(len(dir_lst))])!=True):
                    pass
            if len(visited_cells) == grid_width*grid_height:
                grid[i][j]['color'] = (0, 255, 0)  # Green color in RGB
            draw_grid()
