...
while(len(visited_cells)<grid_width*grid_height): #to check when maze over
    ...
    if current_cell != end_cell:
        grid[current_cell[0]][current_cell[1]]['color'] = (0, 0, 255)  # Blue color in RGB
    visited_cells.append([current_cell[0],current_cell[1]])
    
    if prev != random_start:
        grid[prev[0]][prev[1]]['color'] = (128, 128, 128) # Grey color in RGB
...
if current_cell != end_cell:
    grid[visited_cells[-1][0]][visited_cells[-1][1]]['color'] = (128, 128, 128) # Grey color in RGB
